...
DATA_SAMPLES_LENGTH = 9680000 # 16000 * 605s
# DATA_EMBEDDINGS_LENGTH = 2999
DATA_SAMPLES_PER_GROUP = 320
...
if __name__ == "__main__":
    ...
    # band passed
    # embedding_dir = f"{DATASET_DIR}/20241225/bandPassedDatas"
    # embedding_dir = f"{DATASET_DIR}/2025_0123/bandPassedDatas_h5"
    # 2 labels
    # pretrained_model_path = "2025_trained_model/1layer_model_with_softmax/2_bandPassed/tkiwidia/checkpoints/all_checkpoints/epoch=99-train_loss=0.31801.ckpt"
    lpretrained_model_path = "2025_trained_model/3layer_model_with_softmax/2_bandPassed/zm70mv4r/checkpoints/all_checkpoints/epoch=99-train_loss=0.31648.ckpt"
    data_saving_dir = f"{DATASET_DIR}/newdata_250626"
    embedding_dir = f"{data_saving_dir}/BirdAVES_features"
    aves_label_dir = f"{data_saving_dir}/aves_labels"
    time_label_dir = f"{data_saving_dir}/time_labels"
    # create model
    estimator_network = FeaturesSVMNetwork.load_from_checkpoint(pretrained_model_path, weights_only=True)
    ...
    # # original passed
    embedding_dir = f"{DATASET_DIR}/20241225/originalDatas"
    # 2 labels
    pretrained_model_path = "2025_trained_model/1layer_model_with_softmax/2_original/fw4zptim/checkpoints/all_checkpoints/epoch=99-train_loss=0.31724.ckpt"
    data_saving_dir = f"{DATASET_DIR}/2025_1layer_mode_with_softmax"
    ...
    # avesDim_label_to_timeDim_label(aves_label_dir, time_label_dir)
    # # # 4 labels
    embedding_dir = f"{DATASET_DIR}/ichishima_datas/BirdAVES_features"
    pretrained_model_path = "2025_trained_model/3layer_model_with_softmax/4_original/zctdbho8/checkpoints/all_checkpoints/epoch=199-train_loss=0.74568.ckpt"
    data_saving_dir = f"{DATASET_DIR}/ichishima_datas/BirdAVES_3layer_model_predicted_labels"
    aves_label_dir = f"{data_saving_dir}/BirdAVES_3layer_model_predicted_full_avex_label"
    time_label_dir = f"{data_saving_dir}/BirdAVES_3layer_model_predicted_full_time_label"
    # # create model
    # estimator_network = FeaturesSVMNetwork.load_from_checkpoint(pretrained_model_path)
    # estimator_network.eval()
    # # output label
