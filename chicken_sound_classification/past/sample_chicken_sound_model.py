...
class AvesClassifier(nn.Module):
    ...
    def __init__(self, config_path, model_path, n_classes, trainable, embedding_dim=1024, audio_sr = 16000):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for i in range(batch_size):
          print(f"len x[i]: {len(x[i])}")
          feature = self.model.extract_features(x[i])[0][-1]
          features.append(feature)
        features = torch.stack(features)
        # features = self.model.extract_features(x)[0][-1]
        # print
        # print(f"features: {features}")
        print(f"len features: {len(features)}")
        # print(f"len features[0].mean(dim=1): {len(features[0].mean(dim=1))}")
        # features.append(feature)
        # features = torch.stack(features)
