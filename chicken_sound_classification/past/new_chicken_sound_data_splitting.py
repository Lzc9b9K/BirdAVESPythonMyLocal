...
# SEGMENT_DURATION_S = 3200 # 0.2s
SEGMENT_DURATION_S = 1600 # 0.1s
CALL_TYPES = [
    "Fo", # Food call
    "Pl", # Pleasuse call
    "Di", # Distress call
...
def save_train_and_test_dataset(fulldataset_file, full_dir, train_output_dir, test_output_dir, label_dir):
    train_info = []
    test_info = []
    ...
    for info in train_info:
        # print(info)
        src_path = os.path.join(full_dir, info['filename'])
    ...
    train_df.to_csv(os.path.join(label_dir, '0_train_dataset.csv'), index=False)
    test_df = pd.DataFrame(test_info)
    test_df.to_csv(os.path.join(label_dir, '0_test_dataset.csv'), index=False)
...
if __name__ == "__main__":
    ...
    label_file = "./dataset/total/total_label.csv"
    label_dir = "./dataset/total"

    fulldataset_file = "./dataset/total/full_dataset.csv"
    full_dir = "./dataset/total/full"
    train_dir = "./dataset/total/train"
    test_dir = "./dataset/total/test"
    ...
    # split_audio(audio_file, label_file, full_dir, label_dir)

    # save_train_and_test_dataset(fulldataset_file, full_dir, train_dir, test_dir, label_dir)
