...
from chicken_sound_dataloader import Vox
# from chicken_sound_model import AvesClassifier, set_train_aves, set_eval_aves
from sample_chicken_sound_model import AvesClassifier, set_train_aves, set_eval_aves
from sample_chicken_sound_dataloader import train_loader, val_loader, test_loader
...
def train_one_epoch(model, dataloader, optimizer, loss_function):
    ...
    #       # batch_dict["audio_segment"] = batch_dict["audio_segment"].cuda()
    ...
    #     output = model(batch_dict[0])
    #     loss = loss_function(output, batch_dict[1])
    ...
    for batch in dataloader:
        ...
        # print(f"signals.size : {signals.size()}")
        # print(f"signals[0] : {signals[0]}")
        # print(f"signals[0].size : {signals[0].size()}")
...
def test_one_epoch(model, dataloader, loss_function, epoch_idx):
  ...
  with torch.no_grad():
    for i, batch_dict in enumerate(dataloader):
        ...
        output = model(batch_dict[0])
        # loss = loss_function(output, batch_dict[1])
        all_losses.append(loss_function(output, batch_dict[1]))
...
def run(
  ...
  for epoch_idx in range(n_epochs):
    ...
    test_one_epoch(model, test_loader, loss_function, epoch_idx)
